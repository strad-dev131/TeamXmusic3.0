...
from TEAMXMUSIC.mongo.welcomedb import is_on, set_state, bump, cool, auto_on

BG_PATH = "TEAMXMUSIC/assets/teamx/welcome.png"
FALLBACK_PIC = "TEAMXMUSIC/assets/upic.png"
FONT_PATH = "TEAMXMUSIC/assets/teamx/Arimo.ttf"
...
async def welcome(client, update: ChatMemberUpdated):
    ...
    try:
        ...
        for f in (avatar, img):
            if f and os.path.exists(f) and "TEAMXMUSIC/assets" not in f:
                try:
                    os.remove(f)
                except:
                    pass